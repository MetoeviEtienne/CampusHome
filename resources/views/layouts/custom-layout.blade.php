...
<body class="relative min-h-screen bg-gradient-to-r from-blue-500 to-indigo-600 font-sans">
    ...
    <!-- Particules -->
    @php
        $sizes = ['particle-small', 'particle-medium', 'particle-large'];
    @endphp
    <div aria-hidden="true" class="pointer-events-none fixed inset-0 overflow-hidden -z-10">
        @for ($i = 0; $i < 1000; $i++)
            @php
                $sizeClass = $sizes[array_rand($sizes)];
            @endphp
            <div 
   ...
   <main class="pt-32 flex justify-center items-start min-h-screen px-4">
    <div class="bg-white py-4 px-6 rounded-lg shadow-lg w-full max-w-xl">
        @yield('content')
    </div>
</main>
